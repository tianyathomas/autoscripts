...
class IQIYI:
    ...
    def main(self):
        p00001, p00002, p00003, dfp, qyid = self.parse_cookie(self.cookie)
        try:
...
if __name__ == "__main__":
    ...
    if not cookie:
        print("未设置环境变量 IQIYI_COOKIE，请先在青龙面板添加")
        sendNotify("爱奇艺签到失败", "未设置环境变量 IQIYI_COOKIE")
        exit(1)
    
    print("开始执行爱奇艺签到任务...")
    result = IQIYI(cookie=cookie).main()
    print(result)
    sendNotify("爱奇艺签到", result)
